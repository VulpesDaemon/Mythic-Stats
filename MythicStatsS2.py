...
#Riot API Vars
region = 'NA1'
apikey = 'RGAPI-fdbc2c0d-63be-4c83-be03-3e99902984ba'
gameID = options.game
redID = 200
blueID = 100
...
def main():
    ...
    sheet_name = "Mythic SCS Stats"
    sheet = client.open(sheet_name)
    player_ws = sheet.worksheet(f'Player Results')
    game_ws = sheet.worksheet('Game Results')
    all_player_data = []
    ...
    if options.write:
        ...
        game_ws.append_row([f'{blue_team_name} Vs. {red_team_name}'])
        player_ws.append_row([f'{blue_team_name} Vs. {red_team_name}'])
        game_ws.append_rows([game_info_df.columns.values.tolist()] + game_info_df.values.tolist())
        player_ws.append_rows([player_dataframe.columns.values.tolist()] + player_dataframe.values.tolist())
